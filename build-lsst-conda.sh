...
Optional:
  ...
  --build-spec SPEC     Append SPEC to the conda package version as +SPEC
  --override-rev SPEC   Override an EUPS product with PRODUCT=REPO_URL#REV
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --build-spec) BUILD_SPEC="$2";  shift 2;;
        --override-rev) OVERRIDE_REVS+=("$2"); shift 2;;
        -h|--help)    usage 0;;
...
[[ ${#OVERRIDE_REVS[@]} -gt 0 && -z "$BUILD_SPEC" ]] && {
    echo "ERROR: --build-spec is required when --override-rev is specified"
    usage 1
}
...
if [[ -n "$BUILD_SPEC" ]]; then
    CONDA_VERSION="${CONDA_VERSION}+${BUILD_SPEC}"
fi
...
echo "  Build spec:     ${BUILD_SPEC:-<none>}"
...
if [[ ${#OVERRIDE_REVS[@]} -gt 0 ]] && ! command -v git &>/dev/null; then
...
OVERRIDE_DIR="$BUILD_DIR/overrides"

apply_source_override() {
    local spec="$1"
    local product="${spec%%=*}"
    local repo_and_rev="${spec#*=}"
    local repo="${repo_and_rev%#*}"
    local rev="${repo_and_rev##*#}"
    local checkout="$OVERRIDE_DIR/$product"

    echo "Applying source override: $product ← $repo#$rev"
    git clone "$repo" "$checkout"
    git -C "$checkout" checkout "$rev"
    (
        cd "$checkout"
        setup -r .
    )
...
if [[ ${#OVERRIDE_REVS[@]} -gt 0 ]]; then
    echo "Applying source overrides..."
    mkdir -p "$OVERRIDE_DIR"
    for override in "${OVERRIDE_REVS[@]}"; do
        apply_source_override "$override"
    done
fi
